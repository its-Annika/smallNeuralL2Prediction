...
# === CONFIG ===
TRANSCRIPT_FILE="/fs/nexus-scratch/ashankwi/phonProjectF25/mls_spanish/train/transcripts_25.txt"      # your master transcript file
AUDIO_ROOT="/fs/nexus-scratch/ashankwi/phonProjectF25/mls_spanish/train/audio"               # root directory with nested audio
OUTPUT_DIR="/fs/nexus-scratch/ashankwi/phonProjectF25/spanish_textGrids/train"          # where TextGrids will be saved
MFA_MODEL="spanish_mfa"                   # MFA model (must be downloaded)
...
mfa align "$TMP_DIR" "$MFA_MODEL" "$MFA_MODEL" "$OUTPUT_DIR" --num_jobs "$NUM_JOBS"
